...
fn search_completions(cx: &mut Context, reg: Option<char>) -> Vec<String> {
    let mut items = reg
        ...
        .map_or(Vec::new(), |contents| {
            contents.take(200).map(|value| value.to_string()).collect()
        });
    items.sort_unstable();
    items.dedup();
...
fn search_next_or_prev_impl(cx: &mut Context, movement: Movement, direction: Direction) {
    ...
    let config = cx.editor.config();
    let scrolloff = config.scrolloff;
    if let Some(query) = cx.registers.first('/', cx.editor) {
        let doc = doc!(cx.editor);
        let contents = doc.text().slice(..).to_string();
...
fn make_search_word_bounded(cx: &mut Context) {
    let regex = match cx.registers.first('/', cx.editor) {
        Some(regex) => regex,
        None => return,
    };
    ...
    match cx.registers.push('/', cx.editor, new_regex) {
        Ok(_) => cx.editor.set_status(msg),
        Err(err) => cx.editor.set_error(err.to_string()),
    }
}
...
fn replace_with_yanked(cx: &mut Context) {
    ...
    let repeat = std::iter::repeat(values.last().unwrap());
    let mut values = values
        .iter()
        .chain(repeat)
        .map(|value| Tendril::from(&value.repeat(count)));
...
fn paste(cx: &mut Context, pos: Paste) {
    ...
    let Some(values) = cx.registers.read(reg_name, cx.editor) else { return };
    let values: Vec<_> = values.map(|value| value.to_string()).collect();
    let (view, doc) = current!(cx.editor);
